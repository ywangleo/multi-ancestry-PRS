...
if(bin <= bins){
  if(covfile != "NULL"){
    for(chr in 1:22){
      system(paste0("plink2 --bfile ", bfile_prefix, chr, " --pheno ", phenodir, "/matched_", pheno, "_Bin", bin, ".txt --ci 0.95 --glm cols=chrom,pos,ref,alt,a1freq,beta,se,ci,tz,p,nobs hide-covar  no-x-sex  --vif 1000000 --covar ", covfile, " --covar-variance-standardize --freq --pheno-name ", pheno, " --threads ", n_threads, " --out ", out_prefix, chr, "_", pheno, "_Bin", bin))
    }
  } else{
    for(chr in 1:22){
      system(paste0("plink2 --bfile ", bfile_prefix, chr, " --pheno ", phenodir, "/matched_", pheno, "_Bin", bin, ".txt --ci 0.95 --glm cols=chrom,pos,ref,alt,a1freq,beta,se,ci,tz,p,nobs --freq --pheno-name ", pheno, " --threads ", n_threads, " --out ", out_prefix, chr, "_", pheno, "_Bin", bin))
    }
  }
} else{
