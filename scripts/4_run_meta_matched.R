...
if(bin >=3 & bin <= bins){
  system(paste0("cat ", scriptdir, "/metal-basic.txt >> ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
  for(j in  2:bin){
    if(j <= bin){
      system(paste0("sed -i '$ a PROCESS ", gwasdir, "/", pheno, "_Bin", j, ".glm.linear.gz' ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
    }
  }
  system(paste0("sed -i '$ a OUTFILE ", outdir, "/", pheno, "_Bin2toBin", bin, "_ .TBL'  ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
  system(paste0("sed -i '$ a ANALYZE HETEROGENEITY'  ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
  system(paste0("sed -i '$ a CLEAR'  ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
  
  system(paste0("metal ", scriptdir, "/", pheno, "_Bin2toBin", bin, ".sh"))
  out <- fread(paste0(outdir, "/", pheno, "_Bin2toBin", bin, "_1.TBL"))
  out[,Allele1 := toupper(Allele1)]
  out[,Allele2 := toupper(Allele2)]
  fwrite(out, file = paste0(outdir, "/", pheno, "_Bin2toBin", bin, "_1.TBL.gz"), sep = "\t")
  system(paste0("rm ", outdir, "/", pheno, "_Bin2toBin", bin, "_1.TBL"))
  system(paste0("rm ", outdir, "/", pheno, "_Bin2toBin", bin, "_1.TBL.info"))
} else{
