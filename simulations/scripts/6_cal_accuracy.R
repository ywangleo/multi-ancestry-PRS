...
option_list <- list(
  make_option(c("--pop"), type = "character", default = NULL, help = "Target ancestry"),
    make_option(c("--addpop"), type = "character", default = NULL, help = "added ancestry"),
				...
				if(popfile != "NULL"){
				  ids <- fread(popfile, header = F) ##read target population ids
				    prs <- prs_all[IID %in% ids$V2,]
					  ...
					  names(phen) <- c("FID", "IID", "Y")
					    prs[,PHENO := phen[prs, on = "IID", .(Y)]]
					    } else{
											  ...
											  fit0 <- lm(formula0, data = d)
											    fit1 <- lm(formula1, data = d)
											      r2_inc <- summary(fit1)$adj.r.squared - summary(fit0)$adj.r.squared
											        r2_pval <- anova(fit1, fit0)$'Pr(>F'[2]
												  return(c(r2_inc, r2_pval))
												  }
												  ...
												  res <- data.frame(ldref, pop, addpop, prefix, r2, r2_2.5, r2_97.5, r2_pval, r2_se)
